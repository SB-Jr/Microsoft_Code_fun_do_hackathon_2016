...
false; // navigate across topic boundaries only in design docs
  ...
  $('.video-wrapper > video').removeAttr('controls')
                             .on('click', function() {
    if ($(this.parentElement).hasClass('playing')) {
      this.pause();
      this.removeAttribute('controls')
      $(this.parentElement).removeClass('playing').addClass('paused');
    } else {
      this.play();
      this.setAttribute('controls','')
      $(this.parentElement).addClass('playing').removeClass('paused')
...
(function($) {
  ...
  function makeTogglable() {
    ...
    var contentTitles = h2Titles.filter('#body-content *');
...
(function($) {
  ...
  function updateSupportLibrariesNav(supportLibraryPath, currPath) {
    ...
    if (currPath.indexOf(supportLibraryPath + 'test') > -1) {
      ...
      navTitle = 'Constraint Layout APIs';
    }
